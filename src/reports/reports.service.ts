...
export class ReportsService {
  ...
  async getReportsOverview(workspaceId: string) {
    ...
    const resolvedIssues = allViolations.filter(v => v.severity === 'low').length; // Assuming low severity = resolved
    const pendingIssues = allViolations.filter(v => v.severity === 'high' || v.severity === 'medium').length;
    ...
    // Calculate trends (simplified - comparing current month vs previous month)
    const now = new Date();
    ...
    return {
      complianceMetrics: {
        ...
        trends: {
          ...
          newIssues: criticalIssues + pendingIssues,
          period: 'month'
        }
      },
      recentActivity: {
  ...
  async getReports(workspaceId: string, limit: number = 10, offset: number = 0) {
  ...
  private async generateReportAsync(reportId: string, generateDto: GenerateReportDto) {
    try {
      ...
      const criticalIssues = allViolations.filter(v => v.severity === 'critical').length;
      const resolvedIssues = allViolations.filter(v => v.severity === 'low').length;
      const pendingIssues = allViolations.filter(v => v.severity === 'high' || v.severity === 'medium').length;
      ...
      // Get review comments for rejected versions
      ...
      // Get approved versions with user info
  ...
  async getReportById(reportId: string) {
    ...
    if (report.status === 'completed') {
      ...
      // Get rejected versions with comments
      const allVersions = labels.map(label => label.version).filter(Boolean);
      const rejectedVersionsWithComments = allVersions
        .filter(v => v.status === 'REJECTED' && (v as any).reviewComment)
        .map(v => ({
  ...
  private generateRecommendations(violations: any[], rejectedVersionsWithComments: any[] = []) {
    ...
    if (criticalViolations.length > 0) {
      recommendations.push({
        priority: 'critical',
        action: `Address ${criticalViolations.length} critical violations immediately`,
        impact: 'Will resolve critical compliance issues',
        effort: 'high',
      });
    }
    ...
    if (mediumViolations.length > 0) {
      recommendations.push({
        priority: 'high',
        action: `Standardize processes to reduce ${mediumViolations.length} medium-severity violations`,
        impact: `Will resolve ${mediumViolations.length} medium-severity violations`,
        effort: 'medium',
      });
    }
